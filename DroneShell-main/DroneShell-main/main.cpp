...
using DroneWhatever = SimpleShell<CommandContext>;
...
public:
    ...
    void addVehicleSwitch() {
        this->addSwitch({
            "-v", 
            "",
            "Enter a the vehicle name of the drone you want to control. Names are found in the JSON settings."
            });
    }
...
class ReadyCommand : public DroneCommand {
public: 
    ReadyCommand() : DroneCommand("Ready", "Preps drone for flight")
    {
    ...
    bool execute(const DroneCommandParameters& params) {
        string vehicle_name = getSwitch("-v").toString();
        cout << "Requesting API Control..." << endl;
        ...
        params.context->client.enableApiControl(true, vehicle_name);
        if (params.context->client.isApiControlEnabled()) {
            cout << "Request successful!" << endl;
        }
        else {
            cout << "Request failed! Ending sim..." << endl;
            return false;
        }
        cout << "Arming Drones..." << endl;
        if (params.context->client.armDisarm(true, vehicle_name)) {
            cout << "Armed "<< vehicle_name << "!" << endl;
        }
        else {
            cout << "Failed! Ending sim..." << endl;
            return false;
        }
        params.context->client.takeoffAsync(getSwitch("-timeout_sec").toFloat(), vehicle_name);
        params.context->client.moveToZAsync(-6, 6, 3, yawMode, lookahead, adaptive_lookahead);
        cout << "Ready for flight." << endl;
        return false;
...
class RRTStarMoveCommand : public DroneCommand {
public:
    RRTStarMoveCommand() : DroneCommand("RRTStarMove", "Move the drone based on the path of RRT")
    {
        addYawModeSwitches();
        addDriveTrainSwitch();
        addLookaheadSwitches();
    ...
    bool execute(const DroneCommandParameters& params)
    {
        ...
        if (!params.context->client.isApiControlEnabled()) {
            cout << "Drone is not prepped for flight. Please run ready command." << endl;
            return false;
        }
        ...
        while (!g) {
            ...
            Coord goalCoord = Coord(goal_x, goal_y, goal_z);

            g = rrtStar(startingCoord, goalCoord);
        }
        g->setPath();
        g->printPath();

        std::stack<Node*> final_path = g->getPath();
        ...
        while (!final_path.empty()) {
            Node* next_dest = final_path.top();
            final_path.pop();
            Coord* dest_coord = next_dest->coord;
            std::cout << "Moving to coord:" << dest_coord->x << ", " << dest_coord->y << ", " << dest_coord->z << "\n";
            context->client.moveToPositionAsync(dest_coord->x, dest_coord->y, -1*(dest_coord->z), 5, 2,
                drivetrain, yawMode, lookahead, adaptive_lookahead, vehicle_name);
            std::this_thread::sleep_for(std::chrono::duration<double>(3));
        }
        cout << "Finished pathing!" << endl;
        delete g;
        _CrtDumpMemoryLeaks();
        return false;
    }
...
class RRTStarAutoCommand : public DroneCommand {
public:
    RRTStarAutoCommand() : DroneCommand("RRTStarAuto", "Move the drone based on the path of RRT without user input for goal")
    {
        addYawModeSwitches();
        addDriveTrainSwitch();
        addLookaheadSwitches();
    ...
    void callRRTStar(std::stack<Node*> final_path, const DroneCommandParameters& params) {
        ...
        std::vector<Vector3r> v = generatePath(final_path);
        context->client.moveOnPathAsync(v, 5, 40, drivetrain, yawMode, lookahead, adaptive_lookahead);
  /*      while (!final_path.empty()) {
            Node* next_dest = final_path.top();
            final_path.pop();
            Coord* dest_coord = next_dest->coord;
            std::cout << vehicle_name << ": next to coord :" << dest_coord->x << ", " << dest_coord->y << ", " << dest_coord->z << "\n";
            context->client.moveToPositionAsync(dest_coord->x, dest_coord->y, -1 * (dest_coord->z), 5, 2,
                drivetrain, yawMode, lookahead, adaptive_lookahead, vehicle_name);
            std::this_thread::sleep_for(std::chrono::duration<double>(3));
        }*/
    ...
    std::vector<Vector3r> generatePath(std::stack<Node*> stack_path) {
        std::vector<Vector3r> path;
        while (!stack_path.empty()) {
            Node* next_coord = stack_path.top();
            stack_path.pop();
            Coord* coord = next_coord->coord;
            path.push_back(Vector3r(coord->x, coord->y, -1 * coord->z));
        }
        return path;
    };
    ...
    bool execute(const DroneCommandParameters& params)
    {
        ...
        while (!g) {
            ...
            // Choosing the goal coordinates
            Coord goalCoord = Coord(110.f, 01.f, 5.f);
        ...
        std::stack<Node*> final_path = g->getPath();
        auto f1 = std::async(std::launch::async, &RRTStarAutoCommand::callRRTStar, this, final_path, std::ref(params));
        f1.get();
        return false;
...
int main(int argc, const char *argv[]) {
    ...
    //Add shell commands
    ReadyCommand ready;
    RRTStarMoveCommand rrtstarmove;
    RRTStarAutoCommand rrtstarauto;
    ArmCommand arm;
    ...
    shell.addCommand(ready);
    shell.addCommand(rrtstarmove);
    shell.addCommand(rrtstarauto);
    ...
    while(!shell.readLineAndExecute(&command_context)) {
       /// std::future<bool> f1 = async(launch::async, DroneWhatever::readLineAndExecute, &command_context);
        //status = f1.get();
    }
    _CrtDumpMemoryLeaks();
    return 0;
