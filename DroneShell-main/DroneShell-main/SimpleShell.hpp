    ...
    struct ShellCommandSwitch {
        ...
        string toString() {
            try {
                ...
                throw std::invalid_argument(Utils::stringf("expecting Vehicle name for switch '%s', but found '%s'", name.c_str(), value.c_str()));
            }
        }
    };
...
public:
    ...
    bool execute(const std::string& command_line, ExecContext* const context) {
        ...
        try {
            ...
            //auto f1 = std::async(std::launch::async, &SimpleShell::ShellCommand::execute ,this ,  std::ref(params));
           // auto result = f1.get();
    ...
    //template <class ExecContext>
     bool readLineAndExecute(ExecContext* const context) {
        ...
        if (line.size() > 0) {
            ...
            //auto f1 = std::async(std::launch::async, &SimpleShell::execute, this);
